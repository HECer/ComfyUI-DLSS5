...
Environment: Windows PowerShell, rtk required. The default Python already provides Torch/pytest. Runtime DLLs are tracked with Git LFS. The global Yoke 1.14.0 fingerprint refuses files above 64 MiB; .yoke/tooling/yoke is a local copy whose ONLY compatibility change streams every byte into the same SHA-256 instead of imposing the size cap. Global installation and DLLs untouched. Use node .yoke/tooling/yoke/dist/cli.js for the supervised loop. No guard is bypassed.
