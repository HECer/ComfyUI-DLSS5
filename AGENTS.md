...
For this retained DLSS-1 worktree, continue the existing changes; do not start over. Earlier independent review identified one remaining issue: validate runtime and stage config before creating a new junction; roll back ONLY a newly created junction if final config replacement fails; preserve prior config. Existing fixes for BOM/no-BOM UTF-8, Unicode and deep nested settings must remain. Add/run the targeted regression then full tests. No further inner review round: the outer loop reviews it.

Coordinator repair after independent review: setup now rejects JSON primitives and all arrays, including single-object arrays, before writes. Seven executable PowerShell cases were added: observed RED 6 failed/1 passed, then GREEN 7 passed after actual-type and raw-root validation. Preserve this fix. Finish targeted/full verification and return promptly; do not repeat earlier implementation or investigate Yoke infrastructure. Pytest temp directory access fails in the sandbox; request the normal automatic approval for the exact test command once instead of trying many basetemp directories. The outer supervisor repeats full tests and conducts review.
